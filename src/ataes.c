...
/* ------------------------------------------------------------------------------------
 *	Modified from the original source by Tuwuh Sarwoprasojo, Atmel Bali Team
 *	Changes:
 *  1. Moved tables (sbox, gfmul) to flash/PROGMEM segment for implementation in AVR
 *       Use conditional macro __AVR__ which is defined only if using AVR-GCC.
 *  2. Added some conditional macros to suppress compiler warning on unused function.
 *  Tested to compile and work on
 *  - AVR target: avr-gcc (WinAVR 20100110) 4.3.3
 *  - x86 target: gcc (TDM-2 mingw32) 4.4.1
 *  Default to use precomputed round-key (macro AES_ENC_PREKEYED in aes.h).
 *  On-the-fly keying can be optionally enabled using macro AES_ENC_128_OTFK in aes.h.
 *  On-the-fly keying may yield smaller code compared to precomputed round-key.
 * ------------------------------------------------------------------------------------*/
...
// 111101 Tuwuh
// Add conditional compilation if using avr-gcc: store tables in PROGMEM
// USE_TABLES must be defined to use the tables.
#if defined(__AVR__)
...
// 111101 Tuwuh
// Add conditional compilation if using avr-gcc: store sbox in PROGMEM	
#if defined(__AVR__)
	static const uint_8t sbox[256]  PROGMEM =  sb_data(f1);
	static const uint_8t isbox[256] PROGMEM = isb_data(f1);

	static const uint_8t gfm2_sbox[256] PROGMEM = sb_data(f2);
	static const uint_8t gfm3_sbox[256] PROGMEM = sb_data(f3);

	static const uint_8t gfmul_9[256] PROGMEM = mm_data(f9);
	static const uint_8t gfmul_b[256] PROGMEM = mm_data(fb);
	static const uint_8t gfmul_d[256] PROGMEM = mm_data(fd);
	static const uint_8t gfmul_e[256] PROGMEM = mm_data(fe);

	#define s_box(x)     pgm_read_byte(sbox+(x))
	#define is_box(x)    pgm_read_byte(isbox+(x))
	#define gfm2_sb(x)   pgm_read_byte(gfm2_sbox+(x))
	#define gfm3_sb(x)   pgm_read_byte(gfm3_sbox+(x))
	#define gfm_9(x)     pgm_read_byte(gfmul_9+(x))
	#define gfm_b(x)     pgm_read_byte(gfmul_b+(x))
	#define gfm_d(x)     pgm_read_byte(gfmul_d+(x))
...
// 111027 Tuwuh
// Add conditionals to suppress compiler warning
#if !defined(HAVE_MEMCPY)
...
static void copy_block_nn( void * d, const void *s, uint_8t nn )
{
	...
	// 111012 Tuwuh, cast to a local pointer first
	// With the original code, the compiler raises error: lvalue required as increment operand
	uint_8t* dd = (uint_8t*) d;
	uint_8t* ss = (uint_8t*) s;
...
// 111027 Tuwuh
// Add conditionals to suppress compiler warning
#if defined(AES_DEC_PREKEYED) || defined(AES_DEC_128_OTFK) || defined(AES_DEC_256_OTFK)
...
#endif // defined(AES_DEC_PREKEYED) || defined(AES_DEC_128_OTFK) || defined(AES_DEC_256_OTFK)
...
return_type at_aes_encrypt( const unsigned char in[N_BLOCK], unsigned char  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
...
return_type at_aes_cbc_encrypt( const unsigned char *in, unsigned char *out,
                         int n_block, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    {
        xor_block(iv, in);
        if(at_aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
			return EXIT_FAILURE;
