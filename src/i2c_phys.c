...
uint8_t i2c_select_device_phys(uint8_t device_id) {
  i2c_address_current = device_id & ~1u;
...
__attribute__((weak)) uint8_t i2c_send_bytes(uint8_t  count, const uint8_t * data) {
  ...
  // //	statusCode = i2c_master_write_packet_wait(&i2c_master_instance,
  // &packet);
  // statusCode = i2c_master_write_packet_wait_no_stop(&i2c_master_instance,
  // &packet);
...
__attribute__((weak)) uint8_t i2c_receive_bytes(uint8_t  count, const uint8_t * data) {
  ...
  // statusCode = i2c_send_stop();
  // if (statusCode != I2C_FUNCTION_RETCODE_SUCCESS)
  // 	return I2C_FUNCTION_RETCODE_COMM_FAIL;
