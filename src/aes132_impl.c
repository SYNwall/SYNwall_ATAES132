...
*/
 ...
 uint8_t aes132_read_size(uint8_t *data, uint16_t addr, uint8_t bytesToRead){
	 ...
	 uint8_t ret_code = aes132m_execute(AES132_OPCODE_BLOCK_READ, 0x00, addr, bytesToRead,
	 0, NULL, 0, NULL, 0, NULL, 0, NULL,	g_tx_buffer, g_rx_buffer);	 
	 aes132_print_command_block(ret_code);
	 if (ret_code == AES132_DEVICE_RETCODE_SUCCESS)
	 	memcpy(data, &g_rx_buffer[2], bytesToRead);
...
uint8_t aes132_encrypt_encwrite(uint8_t *in_data, uint8_t in_size, uint8_t *key, uint16_t address, uint8_t in_seed[12])
{
	...
	mac_compute_encrypt_param.param1        = address;
	mac_compute_encrypt_param.param2        = in_size;
	mac_compute_encrypt_param.count_value   = NULL;
	mac_compute_encrypt_param.usage_counter = NULL;
	mac_compute_encrypt_param.serial_num    = NULL;
	mac_compute_encrypt_param.small_zone    = NULL;
